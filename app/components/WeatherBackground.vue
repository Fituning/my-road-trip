<template>
  <div v-if="displayWeather" class="absolute inset-0 pointer-events-none z-0 overflow-hidden">

    <div v-if="displayWeather === 'Clear' && !isNight">
    ...
    <div v-if="displayWeather === 'Clouds' || displayWeather === 'Rain'" class="absolute top-0 left-0 w-full h-64">
      <div class="absolute -top-6 -left-[30%] w-[130%] animate-drift-medium drop-shadow-[0_10px_40px_rgba(0,0,0,0.3)]">
        <svg viewBox="0 0 1000 200" :class="displayWeather === 'Rain' ? 'text-blue-100' : 'text-white'" fill="currentColor" preserveAspectRatio="none"><path d="M0,100 Q80,50 150,100 T300,90 T450,120 T650,80 T850,110 T1000,90 L1000,0 L0,0 Z" /></svg>
      </div>
      <div class="absolute -top-4 -left-[10%] w-[140%] animate-drift-slow drop-shadow-[0_10px_30px_rgba(0,0,0,0.2)]">
        <svg viewBox="0 0 1000 200" :class="displayWeather === 'Rain' ? 'text-blue-200' : 'text-white'" fill="currentColor" preserveAspectRatio="none"><path d="M0,80 Q50,40 100,80 T200,80 T300,60 T450,100 T600,70 T750,90 T900,60 T1000,80 L1000,0 L0,0 Z" /></svg>
      </div>
      <div class="absolute -top-6 -left-[10%] w-[140%] animate-drift-fast drop-shadow-[0_5px_20px_rgba(0,0,0,0.2)]">
        <svg viewBox="0 0 1000 200" :class="displayWeather === 'Rain' ? 'text-slate-300' : 'text-white'" fill="currentColor" preserveAspectRatio="none"><path d="M0,120 Q100,60 200,120 T400,110 T600,140 T800,90 T1000,120 L1000,0 L0,0 Z" /></svg>
      </div>
    </div>
